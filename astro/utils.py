...
def generate_obj_proposals(astro_root, subset, num_proposals, save_path):
    dataset = ASTRO(astro_root, subset=subset, codalab=True)
...
def generate_random_permutation_gt_obj_proposals(astro_root, subset, save_path):
    dataset = ASTRO(astro_root, subset=subset, codalab=True)
    for seq in dataset.get_sequences():
