...
import numpy as np
from astro.astro import ASTRO
from astro.metrics import db_eval_boundary, db_eval_iou
from astro import utils
from astro.results import Results
from scipy.optimize import linear_sum_assignment


class ASTROEvaluation(object):
    def __init__(self, astro_root, task, gt_set, sequences='all', codalab=False):
        """
        Class to evaluate ASTRO sequences from a certain set and for a certain task
        :param astro_root: Path to the ASTRO folder that contains JPEGImages, Annotations, etc. folders.
        ...
        """
        self.astro_root = astro_root
        self.task = task
        self.dataset = ASTRO(root=astro_root, task=task, subset=gt_set, sequences=sequences, codalab=codalab)
