...
import numpy as np
import pandas as pd
from astro.evaluation import ASTROEvaluation

default_astro_path = '/path/to/the/folder/ASTRO'

time_start = time()
parser = argparse.ArgumentParser()
parser.add_argument('--astro_path', type=str, help='Path to the astro folder containing the JPEGImages, Annotations, '
                                                   'ImageSets, Annotations_unsupervised folders',
                    required=False, default=default_astro_path)
parser.add_argument('--set', type=str, help='Subset to evaluate the results', default='test-dev')
parser.add_argument('--task', type=str, help='Task to evaluate the results', default='supervised',
                    choices=['semi-supervised', 'unsupervised', 'supervised'])
